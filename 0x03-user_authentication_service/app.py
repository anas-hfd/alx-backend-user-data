...
def register_user() -> str:
    """Register new user if it does not exist and return JSON payload"""
    try:
...
def log_in() -> str:
    """ Log a user in and return session ID """
    try:
        email = request.form['email']
        password = request.form['password']
    except KeyError:
...
def reset_password() -> str:
    """check if the email is not registered, respond with a 403 status code.
